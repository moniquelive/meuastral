...
This project now deploys through Cloudflare Workers. The worker script in `worker/index.js` serves the static bundle generated by Create Elm App and falls back to `index.html` for SPA routes.

1. Install dependencies: `npm install` (CI environments can run `npm ci` for a clean install).
2. Build the Elm app: `npm run build` (wraps `elm-app build` with `NODE_OPTIONS=--openssl-legacy-provider` so older Webpack works on Node 20+; Cloudflare invokes the same script).
...
- Run `npm test` to execute the Elm suite once in CI mode. The script sets `ELM_HOME=.elm-home`, so compiler caches live within the repo (avoiding permission issues on sandboxed build hosts).
- For watch mode, run `ELM_HOME=.elm-home elm-app test --watch` directly.
